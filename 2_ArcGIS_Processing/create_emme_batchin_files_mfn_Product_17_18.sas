/* CREATE_EMME_BATCHIN_FILES_MFN_PRODUCT_17_18.SAS
   Nick Ferguson, last rev. 5/15/2017 edits scb 12/5/18

-------------                                                             -------------
   THIS PROGRAM CREATES MESO FREIGHT NETWORK BATCHIN FILES FOR THE PRODUCT_17_18 SYSTEM ONLY.
   IT IS CALLED BY CREATE_EMME_BATCHIN_FILES_MFN_PRODUCT_17_18.PY.
-------------                                                             -------------
...
%let outpath=%scan(&sysparm,2,$);
  ...
  proc import datafile="&inpath.\temp_Prod_17_18_nodes.dbf" out=cosnode replace; 
  ...
  proc import datafile="&inpath.\temp_Prod_17_18_System.dbf" out=cosarc replace;
  ...
  proc sort data=arcs; by inode jnode;
         ...
         /* ------------------------------------------------------------------------------ */
                        *** OUTPUT FILES ***;
           filename out2 "&outpath.\p1718_ntwk.txt";run;
         /* ------------------------------------------------------------------------------ */

      * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
             **VERIFY THAT EACH LINK HAS A LENGTH**;
        data check; set arcs(where=(miles=0));
           proc print; title "PRODUCT_17_18 SYSTEM NETWORK LINKS WITHOUT A CODED LENGTH";run;

             **VERIFY THAT EACH LINK HAS A MODE**;
        data check; set arcs(where=(modes is null));
           proc print; title "PRODUCT_17_18 SYSTEM NETWORK LINKS WITHOUT A CODED MODE";
       * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
      ...
      * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
          **VERIFY THAT EACH NODE HAS COORDINATES**;
          data check; set nodes; if point_x='.' or point_y='.';
           proc print; title "PRODUCT_17_18 SYSTEM NETWORK NODES WITH NO COORDINATES";run;
          ...
          **VERIFY THAT EACH NODE HAS A UNIQUE NUMBER**;
          ...
          data check; set check(where=(count>1));
           ...
           title "PRODUCT_17_18 SYSTEM NETWORK NODES WITH DUPLICATE NUMBERS";run;
