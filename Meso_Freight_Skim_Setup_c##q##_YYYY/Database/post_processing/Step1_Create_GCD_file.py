...
inDir2 = os.path.join(databaseDir + '/output_data/'+ scenario)              ##-- Database/output_data/skims
...
pth_mf40= os.path.join(inDir2 + "/mf40.in" )                              ##-- Emme skim file containing POEs used 
...
##-- Variables
maxMeso=273          ##-- Maximum U.S. mesozone number   
maxNatMeso = 491     ##-- Maximum National mesozone number
maxCMAPMeso = 150    ##-- Maximum CMAP mesozone number
...
if qcPC2.shape[0] > 0:                                                 ##-- Stop code if instances occur where reverse does not exist
    print('---> ERROR: Mesozone pairs do not have reverse GCD values')
    print(qcPC2)
    sys.stop()
else:
    print('---> QC CHECK: All mesozone pairs have reverse GCD values')
...
centroids['result'] = centroids['data'].str.replace('a', '', regex=False)       ##-- Replace 'a' in line with ''
centroids['result2'] = centroids['result'].str.replace('*', '', regex=False)    ##-- Replace 'a*' in line with ''
...
mesoCoords=centroids.loc[centroids['node']<=maxMeso].copy()              ##-- Filter out non-centroid nodes and Mexico and Canada (310 and 399)
...
if checkEE.shape[0] > 0:                                                 ##-- Stop code if instances occur where reverse does not exist
    print('---> ERROR: EE POEs entering and exiting the same node')
    print(checkEE)
    sys.stop()
else:
    print('---> QC CHECK: All EE POE trips enter and exit from distinct nodes')
