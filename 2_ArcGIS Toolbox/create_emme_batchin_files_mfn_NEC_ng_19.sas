/* CREATE_EMME_BATCHIN_FILES_MFN_NEC_ng_19.SAS
   Nick Ferguson, last rev. 5/15/2017 edits scb 12/5/2018

-------------                                                             -------------
   THIS PROGRAM CREATES MESO FREIGHT NETWORK BATCHIN FILES FOR THE NEC_ng_19 Network ONLY.
   IT IS CALLED BY CREATE_EMME_BATCHIN_FILES_MFN_NEC_ng_19.PY.
-------------                                                             -------------
...
%let outpath=%scan(&sysparm,2,$);
  ...
  proc import datafile="&inpath.\temp_NEC_NG_19_nodes.dbf" out=cosnode replace; 
  ...
  proc import datafile="&inpath.\temp_NEC_NG_19_System.dbf" out=cosarc replace;
  ...
  proc sort data=arcs; by inode jnode;
         ...
         /* ------------------------------------------------------------------------------ */
                        *** OUTPUT FILES ***;
           filename out2 "&outpath.\batchin\nec_19_ntwk.txt";run;
         /* ------------------------------------------------------------------------------ */

      * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
             **VERIFY THAT EACH LINK HAS A LENGTH**;
        data check; set arcs(where=(miles=0));
           proc print; title "NEC_ng_19 NETWORK LINKS WITHOUT A CODED LENGTH";run;

             **VERIFY THAT EACH LINK HAS A MODE**;
        data check; set arcs(where=(modes is null));
           proc print; title "NEC_ng_19 NETWORK LINKS WITHOUT A CODED MODE";
       * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
      ...
      * - - - - - - - - - - - - - - - - - - - - - - - - - - *;
          **VERIFY THAT EACH NODE HAS COORDINATES**;
          data check; set nodes; if point_x='.' or point_y='.';
           proc print; title "NEC_ng_19 NETWORK NODES WITH NO COORDINATES";run;
          **VERIFY THAT EACH CENTROID HAS COORDINATES**;
          data check; set centroids; if point_x='.' or point_y='.';
           proc print; title "NEC_ng_19 NETWORK CENTROIDS WITH NO COORDINATES";run;
  ...
  data _null_; set centroids;
    file out2;
    if _n_= 1 then put "c NEC_NG_19 NETWORK BATCHIN FILE" /
