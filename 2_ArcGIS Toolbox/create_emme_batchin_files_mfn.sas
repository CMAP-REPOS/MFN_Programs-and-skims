...
%let hwyNode=%scan(&sysparm,3,$);
%let hwyLink=%scan(&sysparm,4,$);
  ...
  proc import datafile="&inpath.\&hwyNode..dbf" out=hwynode1 replace; 
  ...
  data railarc2(keep=inode jnode miles modes type lanes vdf); set railarc2;
	...
	output;
    run;
  proc import datafile="&inpath.\&hwyLink..dbf" out=hwyarc1 replace;
