...
%let outpath=%scan(&sysparm,2,$);
  ...
  proc import datafile="&inpath.\temp_Crude_Oil_System_nodes.dbf" out=cosnode replace; 
  ...
  proc import datafile="&inpath.\temp_Crude_Oil_System.dbf" out=cosarc replace;
  ...
  proc sort data=arcs; by inode jnode;
         ...
         /* ------------------------------------------------------------------------------ */
                        *** OUTPUT FILES ***;
           filename out2 "&outpath.\batchin\cos_ntwk.txt";run;
         /* ------------------------------------------------------------------------------ */
